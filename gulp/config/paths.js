...
const settingPaths = {
  fileExtensions: {
    ...
    images: ["png", "jpg", "jpeg", "gif", "svg", "ico", "webp", "mp4", "avi", "wmv", "mkv", "mov", "flv", "webm", "3gp", "mpeg", "ogg"],
...
// 절대 경로로 설정 하려면 /로 시작하게 하면 됩니다. 예) /assets/styles
// 상대 경로로 설정 하려면 /로 시작하지 않게 하면 됩니다. 예) assets/styles
const projectReplacePaths = {
  ...
  // 필요한 경로를 추가 합니다.
  pathImage: getBuildPath(`assets/images`, `assets/images`),
};
...
const projectPaths = {
  ...
  ftpServerDest: `${settingPaths.folderFtpServer}/${projectDirName}`,

  // html validation
  htmlValidationSrc: [`${destFolder}/**/*.html`],

  // style validation
  styleValidationSrc: [
    `${destFolder}/**/*.css`,
    `!${destFolder}/guide/assets/styles/*.css`,
    `${srcFolder}/assets/styles/layout/*.scss`,
    `${srcFolder}/assets/styles/pages/*.scss`,
    `${srcFolder}/assets/styles/themes/*.scss`,
    `${srcFolder}/assets/styles/utilities/*.scss`,
    `${srcFolder}/assets/styles/components/*.scss`,
    `!${srcFolder}/assets/styles/components/_swiper.scss`,
    `!${srcFolder}/assets/styles/pages/_test.scss`,
    `!${srcFolder}/assets/styles/pages/_codinglist.scss`,
    `!${srcFolder}/assets/styles/pages/_component_guide.scss`,
  ],
};
