...
const postcssBuildConfig = [
  ...
  // combineMediaQuery(), // 동일한 미디어 쿼리를 결합합니다.
];

// 모든 일치 항목을 변환하는 사용자 정의 함수
const replaceAllSvgColors = () => {
  return plugins.replace(
    /(background(?:-image)?:\s*url\("data:image\/svg\+xml[^"]*)/g,
    (match) => {
      return match.replace(/#([0-9a-fA-F]{6})/g, "%23$1");
    },
  );
};
...
const scss = (isBuild) => {
  return (
    gulp
      ...
      // 빌드 시 "no build" 영역 제거
      .pipe(plugins.if(isBuild, plugins.replace(/\/\* no build:start \*\/[\s\S]*?\/\* no build:end \*\//g, '')))
      ...
      // 모든 일치 항목을 변환하도록 사용자 정의 함수 사용
      .pipe(replaceAllSvgColors())
